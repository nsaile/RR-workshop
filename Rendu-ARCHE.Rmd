...
# représenter (en x, en y, en couleurs, en taille...) et un objet géométrique (geom) pour définir sous
# quelle forme nous voulons représenter ces données.
```


4. Qu’est-ce qu’un package ?
```{r}
# C'est un ensemble de fonctions que l'on peut importer, qui ne sont pour la plupart pas intégrées
# de base dans R pour économiser de la place. 
```
...
```{r}
# Les tibble permettent d'afficher des grandes quantités de données plus lisiblement grâce
# au package tibble
```
